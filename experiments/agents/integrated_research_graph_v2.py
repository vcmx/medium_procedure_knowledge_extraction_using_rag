...
# Import RAG components (assuming they're in the same directory or adjust import path)
try:
    from src.modules.embeddings.query_utils import QueryManager
except ImportError:
    # If the module structure is different, you might need to adjust this
    print("Warning: Could not import QueryManager. Make sure the path is correct.")
    QueryManager = None
...
class MultimodalChromaRAGQueryEngine:
    ...
    def __init__(
        ...
        # Initialize LLM
        model_name = (
            llm_model or os.getenv("SELECTED_LLM") or "meta-llama/llama-4-maverick"
        )
    ...
    def query(
        ...
        if experience_years < 3:
            ...
            tone_instruction = (
                "Explain in simple, clear terms for a beginner. Avoid jargon."
            )
            ...
            tone_instruction = (
                "User is advanced. Use concise, expert-level explanation."
            )
...
def rag_query_tool(
    ...
    try:
        ...
        if rag_engine is None:
            return (
                "RAG engine not configured. Please configure RAG in the sidebar first."
            )
...
rag_agent = create_react_agent(llm, tools=[rag_query_tool])
...
def make_supervisor_node(llm: BaseChatModel, members: list[str]):
    ...
    system_prompt = (
        ...
        "Guidelines:\n"
        "- Use 'rag' for questions about technical manuals, documentation, or internal knowledge\n"
        "- Use 'search' for current events, latest information, or topics not in the knowledge base\n"
        "- Use 'coder' for programming tasks, data analysis, or generating charts/visualizations\n"
        "- You can route to multiple agents in sequence if needed\n"
        "- Choose **FINISH** only when the user's request is fully addressed\n\n"
...
def set_rag_config(
    ...
    try:
        ...
        logger.info(
            f"RAG engine successfully configured with directory: {persist_directory}"
        )
        return True
    except Exception as e:
...
if __name__ == "__main__":
    ...
    test_input = {
        "messages": [
            HumanMessage(
                content="Can you search for recent AI developments and also generate a simple Python chart?"
            )
        ]
    }
