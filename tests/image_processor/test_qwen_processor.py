...
from src.modules.image_processor.qwen_local_processor import QwenLocalImageProcessor
...
@pytest.fixture
def qwen_processor(mock_model, mock_processor):
    """Create a Qwen processor instance with mocked model and processor."""
    with (
        patch(
            "src.modules.image_processor.qwen_local_processor.AutoModelForCausalLM.from_pretrained",
            return_value=mock_model,
        ),
        patch(
            "src.modules.image_processor.qwen_local_processor.AutoProcessor.from_pretrained",
            return_value=mock_processor,
        ),
    ):
        processor = QwenLocalImageProcessor()
...
def test_qwen_processor_initialization(mock_model, mock_processor):
    """Test Qwen processor initialization with different device options."""
    with (
        patch(
            "src.modules.image_processor.qwen_local_processor.AutoModelForCausalLM.from_pretrained",
            return_value=mock_model,
        ...
        processor = QwenLocalImageProcessor(device="cpu")
        assert processor.device == "cpu"
        assert processor.torch_dtype == torch.float32


def test_process_image(qwen_processor, sample_image, temp_output_dir):
    """Test processing a single image with mocked model."""
    caption, image_path = qwen_processor.process_image(sample_image, temp_output_dir)
...
def test_process_image_without_saving(qwen_processor, sample_image):
    """Test processing an image without saving it."""
    caption, image_path = qwen_processor.process_image(sample_image)
...
def test_process_batch(qwen_processor, temp_output_dir):
    ...
    results = qwen_processor.process_batch(images, temp_output_dir)
...
def test_real_qwen_processor():
    """Test Qwen processor with real model."""
    processor = QwenLocalImageProcessor()
    assert processor.device in ["cuda", "mps", "cpu"]
    assert processor.processor is not None
    assert processor.model is not None
...
def test_real_image_captioning(temp_output_dir):
    """Test captioning with real Qwen model."""
    processor = QwenLocalImageProcessor()
