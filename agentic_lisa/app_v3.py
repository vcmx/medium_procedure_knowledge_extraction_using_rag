...
from integrated_research_graph_v3 import (
    research_graph,
    set_rag_config,
            ...
            if "messages" in agent_data and agent_data["messages"]:
                for msg in agent_data["messages"]:
                    ...
                    if hasattr(msg, "name") and msg.name:
                        agent_display_name = msg.name.capitalize()
                    elif isinstance(msg, dict) and msg.get("name"):
                        agent_display_name = msg["name"].capitalize()
                    else:
                        agent_display_name = agent_name.capitalize()

                    # Display reasoning or message
                    if (
                        (hasattr(msg, "name") and msg.name == "supervisor")
                        or (isinstance(msg, dict) and msg.get("name") == "supervisor")
                    ):
                        st.markdown("🧭 **Supervisor Decision:**")
                        st.markdown(msg.content if hasattr(msg, "content") else msg.get("content", ""))
                    else:
        ...
        if prompt := st.chat_input("Ask the agents anything (they can search web, run code, or query RAG)"):
            ...
            with st.spinner("Agents are collaborating..."):
                ...
                for step in research_graph.stream(state_input):
                    step_count += 1
                    if st.session_state.agent_history and step == st.session_state.agent_history[-1]:
                        continue
                    ...
                    for agent_name, agent_data in step.items():
                        if "messages" in agent_data and agent_data["messages"]:
                            ...
                            final_response = last_msg.content if hasattr(last_msg, "content") else last_msg.get("content", "")
